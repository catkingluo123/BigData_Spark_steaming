 ...
 */
public class JavaKafkaActive_11203 {

    private static final Logger logger = Logger.getLogger(JavaKafkaActive_11203.class);

    private static final Pattern SPACE = Pattern.compile("\\|");

    private static final String Redis_Hss = "hss_data";

    private static final String Redis_Itc = "cum_11203_";//接口定义

    private static final String Redis_Tot = "tot_act_";

    public static void main(String[] args)  {
        if (args.length < 4) {
            System.err.println("Usage: JavaKafkaActive_11203 <zkQuorum> <group> <topics> <numThreads>");
            System.exit(1);
        }

        SparkConf sparkConf = new SparkConf().setAppName("JavaKafkaActive_11203");
                            ...
                            public Tuple2<Tuple2<String, String>, Integer> call(Tuple2<String, String> v1, Optional<Integer> one, State<Integer> state) throws Exception {
                                ...
                                if (old == 0) {
                                    ...
                                    try {
                                        ...
                                        jedis.pfadd(Redis_Tot + v1._2(), v1._1());//总活跃
                                    }catch (Exception e){
                                        logger.warn(e.toString());
                                    }finally {
                                        if(jedis != null)
